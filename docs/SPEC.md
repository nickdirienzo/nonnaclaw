...
│  │    • mcp__nonnaclaw__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── nonnaclaw.log               # Host stdout
│   └── nonnaclaw.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.nonnaclaw.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'nonnaclaw-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your nonnaclaw installation
...
NonnaClaw uses a hierarchical memory system based on CLAUDE.md files.
...
7. Router invokes Claude Agent SDK:
   ...
   └── mcpServers: nonnaclaw (scheduler)
   │
   ▼
...
NonnaClaw has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__nonnaclaw__schedule_task]
        {
...
### NonnaClaw MCP (built-in)

The `nonnaclaw` MCP server is created dynamically per agent call with the current group's context.
...
NonnaClaw runs as a single macOS launchd service.

### Startup Sequence

When NonnaClaw starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned NonnaClaw containers from previous runs
...
### Service: com.nonnaclaw

**launchd/com.nonnaclaw.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/nonnaclaw.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/nonnaclaw.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.nonnaclaw.plist ~/Library/LaunchAgents/
...
# Check status
launchctl list | grep nonnaclaw

# View logs
tail -f logs/nonnaclaw.log
```
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep nonnaclaw` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; NonnaClaw auto-starts container runtime but may fail |
...
- `logs/nonnaclaw.log` - stdout
- `logs/nonnaclaw.error.log` - stderr
